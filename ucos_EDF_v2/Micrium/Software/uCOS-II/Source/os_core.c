...
void  OSTimeTick(void)
{
    ...
    if (OSRunning == OS_TRUE) {
        ...
        //   // 找到最高優先權的tcb
        //OS_TCB* curTcb = OSTCBList;
        //int maxPriority = 1;
        //OS_TCB* hptcb = OSTCBCur;
        //while (curTcb != NULL) {
        //    if (curTcb->OSTCBPrio < maxPriority && ((curTcb->OSTCBPrio >= 1) && (curTcb->OSTCBPrio <= 2))) {
        //        maxPriority = curTcb->OSTCBPrio;
        //        hptcb = curTcb;
        //    }
        ...
        OS_TCB* hptcb = OSTCBPrioTbl[PERIODIC_TASK_START_PRIO];
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {     /* Go through all TCBs in TCB list              */
            OS_ENTER_CRITICAL();
            ...
            if (ptcb->StartTime <= OSTimeGet()) {
                ...
                if ((ptcb->Deadline < hptcb->Deadline) || ((ptcb->Deadline == hptcb->Deadline) && (ptcb->StartTime < hptcb->StartTime))) {
                    hptcb = ptcb;
